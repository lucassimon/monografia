...
"""
Starter fabfile for deploying the django_rest project.
...
if not hasattr(env, 'prod'):
    ...
    env.hosts = ['@']
    env.project_name = u'django_rest'
    env.shell = u'/bin/zsh -c'
    env.code_dir = u'~/sites/django_rest'
    env.git_repo = u'git@github.com:lucassimon/git'
    env.settings = u'--settings=django_rest.settings.production'
