...
%  tarefas de forma eficiente.\cite{Hughes:2012}
  ...
  \citeonline{Pereira:2013} comparou a execução de dois programas escritos em Node.Js utilizando duas funções nativas
...
%  performance é dada a execução assíncrona que maximizou o processamento reduzindo o tempo total para 200 milesegundos.
  ...
  Através de um outro programa (\ref{leitura-arquivos-diretorio-node}) implementado por \citeonline{Pereira:2013} e referenciado no 
...
%  neste emaranhado de chamadas de retorno.
  ...
  Por fim, como descreve \citeonline{Strongloop:2013}, existe a abordagem utilizando \textit{generators} que estarão
  ...
  Em seu artigo \citeonline{Strongloop:2013} investigou três possibilidades de mitigar o problema dos \textit{callbacks hell}, com o 
...
\label{programacao-orientada-a-eventos}
  ...
  \citeonline{Pereira:2013} compara a orientação de eventos do Node.Js à filosofia de orientação 
...
\label{single-thread}
  ...
  \citeonline{Abernethy:2011} cita que em vez de criar \textit{threads}
  ...
  \citeonline{Powers:2012} cita também a \textit{thread} única como um dos benefícios do ambiente do Node.Js, 
  ...
  Assim como \citeonline{Powers:2012}, \citeonline{Hughes:2012} afirmam que o conceito de \textit{thread} única é importante para 
  ...
  Através de outra biblioteca, \textit{multi-node} \citeonline{Zyp:2010}, é possível compartilhar \textit{sockets} nos servidores.
...
%  conseguindo vantagens e evitando armadilhas dessa abordagem.\cite{Hughes:2012}
  ...
  Aprofundando \citeonline{Pereira:2013} cita o módulo \textit{EventEmitter} como responsável por emitir esses eventos e em 
  ...
  Essa seção é melhor descrita e exemplificada por \citeonline{Wilson:2013} 
...
\label{framework-express}
  ...
  \citeonline{Powers:2012} explica que um \textit{framework} ajuda na infraestrutura e nos permite criar sites e aplicações
  ...
  \citeonline{Pereira:2013} complementa que utilizar a API HTTP nativa do Node.Js pode ser um processo moroso e desgastante
  ...
  \citeonline{Powers:2012} compara o \textit{Express.Js} ao \textit{framework} Sinatra \footnote{Framework escrito em Ruby para criar aplicações Web. 
  ...
  arquivo ou diretório representa. \citeonline{Powers:2012, Hughes:2012} não apresentam um descritivo de cada arquivo 
  ou diretório e seu papel. \citeonline{Pereira:2013, Wilson:2013} tratam mais deste assunto, o que pode ser visto
...
\label{servidor-web-express-js}
  ...
  De acordo com \citeonline{Wilson:2013}, o interessante do Node.Js é que o código do 
